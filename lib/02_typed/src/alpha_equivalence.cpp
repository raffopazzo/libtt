...
struct alpha_equivalence_visitor
{
    ...
    bool operator()(pre_typed_term::app_t const& x, pre_typed_term::app_t const& y) const
    {
        return is_alpha_equivalent(x.left.get(), y.left.get()) and is_alpha_equivalent(x.right.get(), y.right.get());
    }

    bool operator()(pre_typed_term::abs_t const& x, pre_typed_term::abs_t const& y) const
    {
        if (x.var == y.var)
