...
BOOST_AUTO_TEST_CASE(applications_with_function_from_context_dont_reduce)
{
    ...
    ctx = extend(ctx, type::var_t("s")).value();
    ctx = extend(ctx, pre_typed_term::var_t("x"), s).value();
    ctx = extend(ctx, pre_typed_term::var_t("f"), s_to_s).value();
...
BOOST_AUTO_TEST_CASE(can_beta_reduce_inside_abstraction)
{
    ...
    auto const m =
        legal_term(
            type_assign(
                ctx,
                pre_typed_term::abs(pre_typed_term::var_t("x"), s, pre_typed_term::app(id, x))
            ).value());
    BOOST_TEST(num_redexes(m) == 1ul);
    BOOST_TEST(not is_redex(m)); // it contains a redex, but it isn't one itself
    BOOST_TEST(not is_beta_normal(m));
    ...
    BOOST_TEST(is_beta_normal(n));
    BOOST_TEST(beta_normalize(n) == n);
    BOOST_TEST(n.term() == pre_typed_term::abs(pre_typed_term::var_t("x"), s, x));
}

BOOST_AUTO_TEST_CASE(can_beta_reduce_inside_second_order_abstraction)
{
    auto const m =
        legal_term(
            type_assign(
    ...
    BOOST_TEST(is_beta_normal(n));
    BOOST_TEST(beta_normalize(n) == n);
    BOOST_TEST(n.term() == pre_typed_term::abs(type::var_t("s"), pre_typed_term::abs(pre_typed_term::var_t("x"), s, x)));
}
...
BOOST_AUTO_TEST_CASE(beta_normalization_tests)
{
    ...
    BOOST_TEST(
        legal_term(type_assign(ctx, twice).value()).ty() ==
        type::pi("a", type::arr(type::arr(a, a), type::arr(a, a))));
